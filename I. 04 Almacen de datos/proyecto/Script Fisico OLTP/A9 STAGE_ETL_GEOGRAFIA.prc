CREATE OR REPLACE PROCEDURE ETL_STG_GEOGRAFIA IS

  V_NOMBRE_PROCESO   VARCHAR2(30):= 'ETL_STG_GEOGRAFIA';
  V_FEC_INICIO       DATE;
  V_FEC_FIN          DATE;
...
BEGIN
  ...
  -- CODIGO DEL PROCESO
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE STG_GEOGRAFIA';
    ...
    INSERT INTO STG_GEOGRAFIA
    SELECT ADDRESS,CITY,REGION,POSTALCODE,COUNTRY,SEQ_IDW_GEO.NEXTVAL IDW_GEOGRAFIA
    --SELECT DISTINCT(SHIPADDRESS),DISTINCT(SHIPCITY),DISTINCT(SHIPREGION),DISTINCT(SHIPPOSTALCODE),DISTINCT(SHIPCOUNTRY),SEQ_IDW_GEO.NEXTVAL IDW_GEOGRAFIA
